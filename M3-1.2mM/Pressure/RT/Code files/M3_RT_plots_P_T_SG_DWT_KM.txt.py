# ==========================================
# FULL PRESSURE ANALYSIS SCRIPT (robust baseline + safe fits)
# ==========================================
import os
import pandas as pd
import numpy as np
...
from scipy.constants import R as R_SI  # J/(mol*K) — not used for O2 calc below
import pywt

# ------------------------------
# Paths (edit this)
# ------------------------------
data_path_abs = r"C:\Users\chand\Documents\GitHub\Thesis\M3-1.2mM\Pressure\RT\Data files\denoised_pressure_log_RT.csv"
plot_dir      = "./processedplots"
csv_output_dir= "./processedcsv"
...
def load_and_prepare_data():
    df = pd.read_csv(
        data_path_abs,
        usecols=["Elapsed Time (hours)", "ABP2-Sch1 (kPa)", "Date", "Time", "Temperature (C)"]
    )
    df = df[df["Elapsed Time (hours)"] <= 250]

    # coerce numeric
    t   = pd.to_numeric(df["Elapsed Time (hours)"], errors="coerce").to_numpy()
    p   = pd.to_numeric(df["ABP2-Sch1 (kPa)"],     errors="coerce").to_numpy()
    T_C = pd.to_numeric(df["Temperature (C)"],     errors="coerce").to_numpy()

    # drop non-finite rows
    good = np.isfinite(t) & np.isfinite(p) & np.isfinite(T_C)
    t, p, T_C = t[good], p[good], T_C[good]

    date = df["Date"].to_numpy()[good]
    time = df["Time"].to_numpy()[good]
    return t, p, T_C, date, time

# ------------------------------
# Robust manual baseline
# ------------------------------
# ------------------------------
# Robust manual baseline
# ------------------------------
def preprocess_signals_manual_baseline(sample_time, sample_signal,
                                       baseline_start=7.5, baseline_end=9.0,
                                       fallback_minutes=5.0):
    t = np.asarray(sample_time, dtype=float)
    y = np.asarray(sample_signal, dtype=float)   # FIX: safe conversion
    ...
    if (mask.sum() < 3) or (not np.isfinite(y_win).any()):
        # fallback to first N minutes
        fb_end = t.min() + fallback_minutes/60.0
        mask = (t >= t.min()) & (t <= fb_end)
        y_win = y[mask]
        if (mask.sum() < 3) or (not np.isfinite(y_win).any()):
            raise ValueError("Baseline window has no finite samples (original or fallback).")
        baseline = float(np.nanmean(y_win))
        print(f"[Baseline] Fallback {fallback_minutes} min ({t.min():.3f}–{fb_end:.3f} h): {baseline:.4f} kPa")
    else:
        baseline = float(np.nanmean(y_win))
        print(f"[Baseline] Manual {baseline_start}–{baseline_end} h: {baseline:.4f} kPa")

    corrected = y - baseline
    return t, corrected, baseline


# ------------------------------
# Safe uniform grid + interpolation
# ------------------------------
def create_uniform_grid(sample_time, sample_signal, corrected_signal):
    tmin, tmax = float(np.min(sample_time)), float(np.max(sample_time))
    n = max(int((tmax - tmin) * 3600), 10)   # >= 10 points
    uniform_time = np.linspace(tmin, tmax, num=n)

    f_raw  = interp1d(sample_time, sample_signal,  kind="linear", fill_value="extrapolate", assume_sorted=False)
    f_corr = interp1d(sample_time, corrected_signal, kind="linear", fill_value="extrapolate", assume_sorted=False)

    u_raw  = f_raw(uniform_time)
    u_corr = f_corr(uniform_time)

    ok = np.isfinite(uniform_time) & np.isfinite(u_raw) & np.isfinite(u_corr)
    return uniform_time[ok], u_raw[ok], u_corr[ok]

# ------------------------------
# Smoothing (edge-safe SG)
# ------------------------------
...
def fit_first_order_model(time, signal):
    ...
    time, signal = time[ok], signal[ok]
    if len(time) < 3:
        raise ValueError("Not enough finite points to fit first-order model.")
    Pmax  = float(np.nanmax(signal))
    k_init= np.log(2) / max(time[-1] - time[0], 1e-6)
    popt, _ = curve_fit(first_order_model, time, signal, p0=[Pmax, k_init], maxfev=20000)
    return popt, first_order_model(time, *popt), time

def logistic_model(t, t0, k, P_max):
    return P_max / (1 + np.exp(-k * (t - t0)))
...
def fit_logistic_model(time, signal):
    ...
    time, signal = time[ok], signal[ok]
    if len(time) < 5:
        raise ValueError("Not enough finite points to fit logistic model.")

    P_max = float(np.nanmax(signal))
    t0_init = float(time[np.argmin(np.abs(signal - P_max/2))])
    k_init = 1.0 / max(time[-1] - time[0], 1e-6)

    bounds = [(time[0], time[-1]), (0.0, 1.0), (0.5*P_max, 1.5*P_max)]
    res = minimize(
        lambda p: mean_squared_error(signal, logistic_model(time, *p)),
        x0=[t0_init, k_init, P_max],
        bounds=bounds
    )
    popt = res.x
    return popt, logistic_model(time, *popt), time
...
# 2) Robust baseline (manual 7.5–9 h; fallback 5 min if unavailable)
sample_time, corrected_signal, baseline = preprocess_signals_manual_baseline(
    sample_time, sample_signal, baseline_start=7.5, baseline_end=9.0, fallback_minutes=5
)

# 3) Uniform grid
uniform_time, u_raw, u_corrected = create_uniform_grid(sample_time, sample_signal, corrected_signal)

# 4) SG + DWT
u_corrected_smooth = smooth_signal(u_corrected, window=11, polyorder=3)
dwt_signal = dwt_denoise(u_corrected_smooth)

# 5) Interpolate temperature to grid and align lengths
temp_interp = interp1d(sample_time, sample_temp, kind='linear', fill_value='extrapolate')
temperature_aligned = temp_interp(uniform_time)

L = min(len(uniform_time), len(dwt_signal), len(temperature_aligned), len(u_corrected))
uniform_time = uniform_time[:L]
dwt_signal   = dwt_signal[:L]
temperature_aligned = temperature_aligned[:L]
u_corrected  = u_corrected[:L]

# Safety mask (strip any residual non-finite)
ok_all = np.isfinite(uniform_time) & np.isfinite(dwt_signal) & np.isfinite(temperature_aligned)
uniform_time, dwt_signal, temperature_aligned, u_corrected = (
    uniform_time[ok_all], dwt_signal[ok_all], temperature_aligned[ok_all], u_corrected[ok_all]
)

# 6) Fit models (safe)
popt_first, fitted_first, t_first = fit_first_order_model(uniform_time, dwt_signal)
popt_logi,  fitted_logi,  t_logi  = fit_logistic_model(uniform_time, dwt_signal)
...
def fit_metrics(y, yhat, k_params):
    ...
    return dict(
        ...
        R2=r2_score(y, yhat),
        AIC=aic(n, rss, k_params),
        BIC=bic(n, rss, k_params)
    )

m_first = fit_metrics(dwt_signal[:len(fitted_first)], fitted_first, 2)
m_logi  = fit_metrics(dwt_signal[:len(fitted_logi)],  fitted_logi,  3)

print("\n=== Model Fit Summary ===")
print(pd.DataFrame([
    {"Model":"First-order", **m_first, "Half-Life (h)": np.log(2)/popt_first[1]},
    {"Model":"Logistic",    **m_logi,  "Half-Life (h)": popt_logi[0]}
]).round(4).to_string(index=False))

# ------------------------------
# Plots (lightweight)
# ------------------------------
colors = {"dwt":"#2c7bb6", "raw":"#999999", "first":"#009F82", "logi":"#d95f02"}

fig1, ax1 = plt.subplots(figsize=(6.75, 5), dpi=dpi)
ax1.plot(uniform_time, u_corrected, color=colors["raw"], lw=2, alpha=0.35, ls="--", label="Raw (baseline-corrected)")
ax1.plot(uniform_time, dwt_signal,  color=colors["dwt"], lw=2.5, label="Denoised pressure")
ax1.plot(t_first, fitted_first, ls="--", color=colors["first"], lw=2, label="First-order fit")
ax1.plot(t_logi,  fitted_logi,  ls="-",  color=colors["logi"],  lw=2, label="Logistic fit")

ax1.set_xlabel("Elapsed Time (h)", fontsize=16)
...
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "RT_pressure_denoised_and_fits.png"), dpi=dpi)

# ------------------------------
# O2 release from pressure (use DWT)
# ------------------------------
...
V_headspace_L = 0.004
R_gas_atm     = 0.082057  # L·atm·mol^-1·K^-1  (distinct from scipy.constants.R)

pressure_atm  = dwt_signal / 101.325
temperature_K = temperature_aligned + 273.15

o2_micromol   = ((pressure_atm * V_headspace_L) / (R_gas_atm * temperature_K)) * 1e6
ratio         = volume_solution_L / V_headspace_L

df_o2_export = pd.DataFrame({
    "Sample": "1.2 mM @ RT",
    "Time (h)": uniform_time,
    "DWT denoised pressure (kPa)": dwt_signal,
    "calibrated temperature (C)": temperature_aligned,
    "O2 Released (µmol)": o2_micromol,
    "Smoothed Pressure (kPa)": u_corrected,
    "% Conversion": (o2_micromol / max_o2_umol) * 100,
    "Soln/Headspace Ratio": ratio,
    "Max O2 Possible (µmol)": max_o2_umol
})
df_o2_export.to_csv(os.path.join(csv_output_dir, "M3_1.2mM_RT_pressure_o2_release.csv"), index=False)

fig2, ax2 = plt.subplots(figsize=(6.75, 5), dpi=dpi)
ax2.plot(uniform_time, o2_micromol, color=colors["dwt"], lw=2.5, label=r"O$_2$ released")
ax2.set_xlabel("Elapsed Time (h)", fontsize=16)
ax2.set_ylabel(r"O$_2$ Released ($\mu$mol)", fontsize=16)
ax2.grid(axis='y', linestyle='--', alpha=0.7)
ax2.legend(frameon=False, fontsize=12, loc="lower right")
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "RT_o2_release.png"), dpi=dpi)

print("\n[OK] CSV saved:", os.path.join(csv_output_dir, "M3_1.2mM_RT_pressure_o2_release.csv"))
print("[OK] Plots saved in:", plot_dir)
...
#         os.path.join(data_dir, r"C:\Users\chand\Documents\GitHub\Thesis\M3-1.2mM\Pressure\RT\Data files\denoised_pressure_log_RT.csv"),
#         usecols=["Elapsed Time (hours)", "ABP2-Sch1 (kPa)", "Date", "Time", "Temperature (C)"]
#     )
#     # sample_df = sample_df[sample_df["Elapsed Time (hours)"] > 25 / 60]
#     sample_df = sample_df[sample_df["Elapsed Time (hours)"] <= 250]

#     sample_time = sample_df["Elapsed Time (hours)"].values
#     sample_signal = sample_df["ABP2-Sch1 (kPa)"].values
...
# def preprocess_signals_manual_baseline(sample_time, sample_signal,
#                                        baseline_start=7.5, baseline_end=9):
#     baseline_mask = (sample_time >= baseline_start) & (sample_time <= baseline_end)
#     # if np.sum(baseline_mask) < 10:
#     #     raise ValueError("Too few points for manual baseline.")
#     baseline = np.mean(sample_signal[baseline_mask])
#     corrected_signal = sample_signal - baseline
#     print(f"Manual baseline from {baseline_start}–{baseline_end} h: {baseline:.4f} kPa")
...
# # === MAIN EXECUTION ===
# sample_time, sample_signal, sample_temp, sample_date, sample_clock = load_and_prepare_data()
# corrected_signal, baseline = preprocess_signals_manual_baseline(sample_time, sample_signal)
...
# plt.tight_layout()
# plt.savefig(os.path.join(plot_dir, "0_ABP2_plot1_M3.png"), dpi=dpi)
...
# plt.tight_layout()
# os.makedirs(plot_dir, exist_ok=True)
# plt.savefig(os.path.join(plot_dir, "M3_ABP2_temp_colored_pressure_fit.png"), dpi=dpi)


# # === Plot2B: Temperature-colored pressure profile with raw overlay (Sample M3) ===
# fig, ax = plt.subplots(figsize=(6.75, 5), dpi=dpi)
...
# plt.tight_layout()
# os.makedirs(plot_dir, exist_ok=True)
# plt.savefig(os.path.join(plot_dir, "ACSOmega_M3_ABP2_temp_colored_pressure_fit.png"), dpi=dpi)
...
# plt.tight_layout()
# plt.savefig(os.path.join(plot_dir, "M3_ABP2_kinetic_model_fits.png"), dpi=dpi)
...
# plt.tight_layout()
# plt.savefig(os.path.join(plot_dir, "M3_ABP2_temp_dependent_kinetic_model_fits.png"), dpi=dpi)
...
# plt.tight_layout()
# plt.savefig(os.path.join(plot_dir, "M3_ABP2_wavelet_decomposition.png"), dpi=dpi)
...
# plt.tight_layout()
# plt.savefig(os.path.join(plot_dir, "M3_ABP2_sg_window_score_plot.png"), dpi=dpi)
...
# # ==== plot 7 ====
# def plot_snr_rmse_comparison(df, optimal_window=None, save_path="M3_ABP2_SI_SNR_RMSE_vs_Window.png"):
...
# pressure_atm = dwt_signal / 101.325       # Convert pressure (kPa) to atm 
# temperature_K = temperature_aligned + 273.15  # temperature to Kelvin
...
# # === Plotting the O2 release curve ===
# fig, ax = plt.subplots(figsize=(6.75, 5), dpi=dpi)

# ax.plot(uniform_time, o2_micromol,
#         color=colors["dwt"], lw=3, label=fr"O$_2$ released")

# ax.set_xlabel("Elapsed Time (h)", fontsize=18)
# ax.set_ylabel(fr"O$_2$ Released ($\mu$mol)", fontsize=18)

# # Add a twin axis for % conversion
# ax2 = ax.twinx()
# ax2.plot(uniform_time, (o2_micromol / max_o2_umol) * 100,
#          color='gray', lw=2, linestyle='--', alpha=0.6, label="\% Conversion")
# ax2.set_ylabel("\% Conversion", fontsize=18)
# ax2.set_ylim(0, 100)

# ax.set_title(fr"Oxygen Release from Pressure Measurements", fontsize=18)
# ax.grid(axis='y', linestyle='--', alpha=0.7)
...
# plt.tight_layout()
# plt.savefig(os.path.join(plot_dir, "M3_ABP2_o2_release_curve.png"), dpi=dpi)