...
main = r"C:\Users\chand\Documents\GitHub\Thesis\M4-1.6 mM\RT\Raw_data\merged_pressure_log.csv"
...
def load_and_prepare_data():
    sample_df = pd.read_csv(main,
        usecols=["Elapsed Time (hours)", "ABP2-Sch1 (kPa)", "Date", "Time", "Temperature (C)"]
    )
    sample_df = sample_df[(sample_df["Elapsed Time (hours)"] > 40) & (sample_df["Elapsed Time (hours)"] <= 250)].reset_index(drop=True)
...
def preprocess_signals_manual_baseline(sample_time, sample_signal,
                                       baseline_start=40, baseline_end=41.5):
...
sample_time = sample_time - sample_time.min()
corrected_signal, baseline = preprocess_signals_manual_baseline(sample_time, sample_signal, baseline_start=0.0, baseline_end=1.5)
...
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "0_ABP2_plot1_M4_RT.png"), dpi=dpi)
...
plt.savefig(os.path.join(plot_dir, "M4_RT_ABP2_temp_colored_pressure_fit.png"), dpi=dpi)
...
plt.tight_layout()
os.makedirs(plot_dir, exist_ok=True)
plt.savefig(os.path.join(plot_dir, "ACSOmega_M4_RT_ABP2_temp_colored_pressure_fit.png"), dpi=dpi)
...
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "M4_RT_ABP2_kinetic_model_fits.png"), dpi=dpi)
...
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "M4_RT_ABP2_temp_dependent_kinetic_model_fits.png"), dpi=dpi)
...
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "M4_RT_ABP2_wavelet_decomposition.png"), dpi=dpi)
...
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "M4_RT_ABP2_sg_window_score_plot.png"), dpi=dpi)
...
# ==== plot 7 ====
def plot_snr_rmse_comparison(df, optimal_window=None, save_path="M4_RT_ABP2_SI_SNR_RMSE_vs_Window.png"):
...
pressure_atm_sg = u_corrected / 101.325

o2_micromol_sg = ((pressure_atm_sg * V_headspace_L) / (R_gas_atm * temperature_K)) * 1e6
...
df_o2_export = pd.DataFrame({
    ...
    "SG Smoothed Pressure (kPa)": u_corrected,
    "O2 Released with SG (µmol)": o2_micromol_sg,
    "% Conversion": (o2_micromol / max_o2_umol) * 100,
    "Soln/Headspace Ratio": ratio,
    "Max O2 Possible (µmol)": max_o2_umol
})
df_o2_export.to_csv(os.path.join(csv_output_dir, "M4_1.6mM_RT_pressure_o2_release.csv"), index=False)
...
print("\n[OK] CSV saved:", os.path.join(csv_output_dir, "M4_1.6mM_RT_pressure_o2_release.csv"))
